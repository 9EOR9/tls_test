/* 
  ...
  Requirement: Windows build >= 20348
...
int sendBuffer(int sockfd, const char* string, size_t length)
{
  char* buffer;
  int bufsize;
  int rc;
...
static int tls_connect(tls_socket* s, const char* hostname, unsigned short port, const char *cert_file, const char *key_file)
{
  CERT_CONTEXT* cert_ctx = NULL;
    ...
    /* Read server packet */
    if (recv(s->sock, buf, 1024, 0) <= 0)
    {
      printf("Error reading server greeting packet\n");
      return -1;
    }

    /* Set TLS capability and send it to server */
    /* CLIENT_DEFAULT_FLAGS + CLIENT_SSL */
    unsigned int client_flags = 3768317919 + 2048;
    ...
    *p++ = 0x21;
    ...
    if (sendBuffer(s->sock, buf, p - buf) < 0)
    {
      printf("Sending MariaDB client hello packet failed\n");
      return -1;
    }
    ...
    // initialize schannel
    {
      ...
      {
        .grbitDisabledProtocols = SP_PROT_TLS1_0_CLIENT | SP_PROT_TLS1_1_CLIENT |
                                 SP_PROT_TLS1_2_CLIENT,
        .cDisabledCrypto = 0,
        .pDisabledCrypto = NULL,
        .cAlpnIds = 0,
        .pDisabledCrypto= NULL,
        .rgstrAlpnIds= NULL
      };
      ...
      cred.dwFlags = SCH_USE_STRONG_CRYPTO | SCH_CRED_MANUAL_CRED_VALIDATION |
        SCH_CRED_NO_DEFAULT_CREDS;
      cred.cTlsParameters = 1;
      cred.pTlsParameters = &tls_params;
     
      if ((cert_file || key_file) && (cert_ctx = schannel_create_cert_context(cert_file, key_file, errmsg, 128)))
      {
        cred.cCreds = 1;
        cred.paCred = &cert_ctx;
      }
      
      if ((rc= AcquireCredentialsHandleA(NULL, UNISP_NAME_A, SECPKG_CRED_OUTBOUND, NULL, &cred, NULL, NULL, &s->handle, NULL)) != SEC_E_OK)
      {
        closesocket(s->sock);
        WSACleanup();
        return -1;
...
{
    const char* hostname = "db4free.net";
    

    tls_socket s;
    if (tls_connect(&s, hostname, 3306, argc > 1 ? argv[1] : NULL, argc > 2 ? argv[2] : NULL) != 0)
    {
